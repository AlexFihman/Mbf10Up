...
const int DIMENSION =  11;// dimension should be between 2 and 11. For bigger dimensions we are running into arithmetic overflows.
...
// precomputed strict supersets for each item (all j > item with (j & item) == item)
extern int superset_count[NUM_BITS];
extern int supersets[NUM_BITS][NUM_BITS];
